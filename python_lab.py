...
S_intersect_T = { S for S in {1, 2, 3, 4} if S in {3, 4, 5, 6} }
...
LofL_sum = sum({sum(n) for n in LofL})
...
S = {-4, -2, 1, 2, 5, 0}
# Replace [ ... ] with a one-line list comprehension in which S appears
exclude_zero_list = [x for x in [(i, j, k) for i in S for j in S for k in S if sum((i, j, k)) == 0] if x != (0, 0, 0)]
...
first_of_tuples_list = [x for x in [(i, j, k) for i in S for j in S for k in S if sum((i, j, k)) == 0] if x != (0, 0, 0)][0]
...
square_dict = {a: a ** 2 for a in range(100)}
...
representation_dict = {a: (x,y,z) for a in range(1000) for x in digits for y in digits for z in digits if x * base ** 2 + y * base + z == a}
...
listdict2dict = {a:b for (z,a) in list(zip(id2salary.keys(), names)) for b in id2salary.values()}       #maps all three keys to 990
